...
You will need to upgrade your MythTV install to at least a v0.%d+Fixes release.'''),
        ...
        try:
            ...
            if self.OWN_VERSION[0] >= 29:
                l = list(OWN_VERSION)
                l[0] = 0
                l[1] = self.OWN_VERSION[0]
                self.OWN_VERSION = tuple(l)
            #
            self.configuration['mythtv_version'] = '.'.join([str(i) for i in OWN_VERSION])
            #
            if self.OWN_VERSION[0] == 0:
                pre = '0.'
            else:
                pre = 'pre-0.'
            self.configuration['mythtv_version'] = u'%s%d.%d' % (pre, self.OWN_VERSION[1], self.OWN_VERSION[3])
            #
            ## Lossless Cut only supports MythTV v0.24+fixes and higher
            if self.OWN_VERSION[1] < common.SUPPORTED_VERSIONS:
                verbage = self.error_messages['VersionError1'] % self.configuration['mythtv_version']
            ...
            if self.OWN_VERSION[1] <= common.UNSUPPORTED_PRE_VERSION and not pre.find('pre-') == -1:
                verbage = self.error_messages['VersionError2'] % (self.configuration['mythtv_version'], self.OWN_VERSION[1])
                self.logger.critical(verbage)
                raise Exception(verbage)
            #
            ...
            try:
                ...
                if not os.path.isfile(filename):
                    verbage = self.error_messages['ConfigMissing'] % filename
                    self.logger.critical(verbage)
                    raise Exception(verbage)
                else:
                    verbage = self.error_messages['ConfigError'] % filename
                    self.logger.critical(verbage)
                    raise Exception(verbage)
            ...
            if self.mythdb:
                ...
                self.localhostname = '10.230.1.15'
                try:
                    self.mythbeconn = self.MythBE(backend=self.localhostname, db=self.mythdb)
                except self.MythError as errmsg:
                    verbage = self.error_messages['BackendConnectionAttempt'] % errmsg.args[0]
                    self.logger.critical(verbage)
                    raise Exception(verbage)
        ...
        format_str = self.mythdb.settings[self.localhostname]['mythvideo.TVexportfmt']
        if format_str:
            self.configuration['TVexportfmt'] = format_str
        format_str = self.mythdb.settings[self.localhostname]['mythvideo.MOVIEexportfmt']
        if format_str:
            self.configuration['MOVIEexportfmt'] = format_str
        format_str = self.mythdb.settings[self.localhostname]['mythvideo.GENERICexportfmt']
        if format_str:
    ...
    def get_recorded_data(self, ):
        ...
        #
        recorded = list(self.mythdb.searchRecorded(basename=self.configuration['base_name']))
        if not recorded:
            # TRANSLATORS: Please leave %s as it is,
            # because it is needed by the program.
            # Thank you for contributing to this project.
            verbage = _(
u'''There is no MythTV recorded record for video file "%s", aborting script.''') % self.configuration['base_name']
            self.logger.critical(verbage)
...
#            exit(int(common.JOBSTATUS().ABORTED))
        ...
        if self.OWN_VERSION[1] == 24:
            self.configuration['inetref'], self.configuration['season_num'], self.configuration['episode_num'] = None, None, None
        else:
        ...
        self.configuration['fps'], self.configuration['first_frame'], self.configuration['last_frame'], self.configuration['width'], self.configuration['height'] = self._get_fps_and_more()
        #
        ## Get missing metadata using the grabbers
        if self.configuration['subtitle'] or (self.configuration['episode_num'] and self.configuration['inetref']):
            self._get_tv_metadata(series=self.configuration['title'], episode=self.configuration['subtitle'], inetref=self.configuration['inetref'], season_num=self.configuration['season_num'], episode_num=self.configuration['episode_num'])
            #
            if self.metadata:
                ...
                self.configuration['episode'] = self.metadata['subtitle']
            ...
            if self.metadata:
                ...
                if self.metadata['releasedate']:
                    self.configuration['releasedate'] = self.metadata['releasedate'].strftime('%Y')
                if self.metadata['description']:
                    self.configuration['description'] = self.metadata['description']
        #
        ## Set the mkv metadata
        if self.configuration['subtitle']:
            if self.configuration['season_num'] and self.configuration['episode_num']:
        ...
        self.configuration['mkv_description'] = self.configuration['description'].replace(u'"', u"'")
        #
        ...
        if self.recorded_program and self.recorded_program.category_type == 'series':
            if self.configuration['subtitle'] and self.configuration['episode_num'] > 0:
                ...
                self.configuration['export_format'] = self.configuration['GENERICexportfmt']
            ...
            if self.configuration['releasedate']:
                ...
                self.configuration['export_format'] = self.configuration['MOVIEexportfmt'].replace(u'(%(releasedate)s)', u'').strip()
        else:
            self.configuration['export_format'] = self.configuration['GENERICexportfmt']
        #
        ...
        if self.configuration['export_format'].find('%GENRE%') != -1 and not self.configuration['genre'] and self.configuration['mythvideo_export']:
...
#
    def _get_cutlist_data(self, ):
        ...
        #
        if not self.configuration['rawcutlist'] and self.configuration['gencutlist']:
            ## Generate the cutlist
            arguments = ('--gencutlist ' + common.GEN_GET_CUTLIST_CMD) % self.configuration
...
%s''') % (self.configuration['mythutil'], self.configuration['mythutil'], arguments, stdout))
            if not result[0]:
                # TRANSLATORS: Please leave %s as it is,
                # because it is needed by the program.
                # Thank you for contributing to this project.
...
Error: %s''') % (self.configuration['mythutil'], result[1])
            ...
            self.recorded = list(self.mythdb.searchRecorded(basename=self.configuration['base_name']))[0]
            #
            ## Now the markup information is accurate
            self.configuration['rawcutlist'] = self.recorded.markup.getcutlist()
        #
        if not self.configuration['rawcutlist']:
            self.configuration['first_last_keyframes'] = self._process_cutlist(first_last=True)
        #
        self.configuration['pre_massage_cutlist'] = self.recorded.markup.getuncutlist()
...
#
    def _get_fps_and_more(self, ):
        ...
        fps = None
        try:
            fps = int(self.configuration['trackinfo']['video_track_details']['Original_frame_rate'])
        except (ValueError, TypeError):
            pass
        #
        height = None
        try:
            height = int(self.configuration['trackinfo']['video_track_details']['Height'])
        except (ValueError, TypeError):
            pass
        #
        width = None
        try:
            width = int(self.configuration['trackinfo']['video_track_details']['Width'])
        except (ValueError, TypeError):
            pass
        #
...
#
    def _process_cutlist(self, first_last=False):
        ...
        if first_last:
            cut_frames = [[self.configuration['first_frame'], self.configuration['last_frame']]]
        else:
        ...
        for start_end in cut_frames:
            if start_end[0] >= self.configuration['last_frame'] or start_end[1] == 0:
                continue
            #
            start, end = 0.0, 0.0
            ...
            if start_end[0] == 0:
                if self.keyframe_adjust:
                    ...
                    self.configuration['frame'] = self.configuration['first_frame'] + 1
                    start = greater_than(self.configuration['frame'])
            elif start_end[0]:
                if self.keyframe_adjust:
                    if first_frame_zero:
                        self.configuration['frame'] = start_end[0] + self.markup_frame_difference
                        start = less_than(self.configuration['frame'])
                    else:
                        self.configuration['frame'] = start_end[0] - self.markup_frame_difference
                        start = greater_than(self.configuration['frame'])
            ...
            if start_end[1]:
                ...
                if self.keyframe_adjust and not first_frame_zero and start_end[1] <= self.configuration['first_frame'] or \
                   start_end[1] == self.configuration['first_frame'] + self.markup_frame_difference:
                    self.configuration['frame'] = self.configuration['first_frame']
                elif self.keyframe_adjust:
                    self.configuration['frame'] = start_end[1] - self.markup_frame_difference
                elif not first_frame_zero and start_end[1] <= self.configuration['first_frame']:
                    self.configuration['frame'] = self.configuration['first_frame'] + 1
                elif not first_frame_zero:
                    self.configuration['frame'] = start_end[1] - self.markup_frame_difference - 1
                else:
                    self.configuration['frame'] = start_end[1] - self.markup_frame_difference - 1
                #
...
%s
''' % (self.configuration['mythutil'], self.configuration['mythutil'], arguments, stdout)))
        if not result[0]:
            # TRANSLATORS: Please leave %s as it is,
            # because it is needed by the program.
            # Thank you for contributing to this project.
...
''') % (self.configuration, result[1])
        ...
        self.recorded.basename = self.configuration['recorded_name'] + u'.mkv'
        #
        ...
        self.recorded.markup[:] = [mark for mark in self.recorded.markup
                                    if not mark['type'] in [-10, -2, 2, 3, 6, 7, 8 , 9, 34 ] ]
        #
...
#
    def _get_new_duration(self,):
        ...
        ## Get the mkv video's duration for the recorded markup
        ## type 33 record update
        result = commandline_call('mkvinfo', u'"%s"' % self.configuration['mkv_file'])
        stdout = u''
        if self.configuration['verbose']:
            stdout = result[1]
        self.logger.info(
...
Error: %s''') % (result[1])
        ...
        if index == -1:
            ...
            # because it is needed by the program.
            # Thank you for contributing to this project.
            verbage = _(u'''mkvinfo could not find the mkv video's duration, aborting script.''')
            self.logger.critical(verbage)
            self.stderr.write(verbage + u'\n')
            #
            ## Remove this recording's files from the working directory
        ...
        self.configuration['new_duration'] = int(result[1][index:index2].replace(u'.', u''))
        #
        return
#
...
file "%s", aborting script.''') % self.configuration['base_name']
        ...
        self.configuration['progstart'] = self.recorded.progstart
        #
        if 'SQL_starttime' not in self.configuration or 'SQL_progstart' not in self.configuration:
            self._set_sql_starttime()
        #
...
file "%s", nothing for this script to do.''') % self.configuration['base_name']
        ...
        for frame_list in [skiplist, cutlist]:
            ...
            #
            if not frame_list:
                verbage = _(u'''The %s is empty''') % which_list
                self.logger.info(verbage)
                self.stderr.write(verbage + u'\n')
                skip_done = True
                continue
            ...
            for count in range(len(self.configuration['keyframe_cuts'])):
                for startend in range(0, 2):
                    if (self.configuration['keyframe_cuts'][count][startend] == frame_list[count][startend]) or \
                       (self.configuration['keyframe_cuts'][count][startend] + self.markup_frame_difference == frame_list[count][startend]):
                        continue
                    if startend == 1 and frame_list[count][startend] == 9999999:
                        continue
                    self.configuration['keyframe_cuts'][count][startend] += self.markup_frame_difference
                    change = True
            if not change:
                verbage = _(u'''The %s already has keyframe values''') % which_list
                self.logger.info(verbage)
                self.stdout.write(verbage + u'\n')
                skip_done = True
                continue
...
%s:            %s
%s keyframes:  %s
''') % (which_list, which_list, frame_list, which_list, self.configuration['keyframe_cuts'])
            self.logger.info(verbage)
            self.stdout.write(verbage + u'\n')
            #
            for count in range(len(self.configuration['keyframe_cuts'])):
                for count in range(len(frame_list)):
                    for startend in range(0, 2):
                        for markuptype in startend_types:
                            ...
                            key_mark = self.configuration['keyframe_cuts'][count][startend]
                            for markup in self.recorded.markup:
                                if markup['mark'] == mark and markup['type'] == markuptype:
...
%s
''' % (self.configuration['mythutil'], self.configuration['mythutil'], arguments, result[1])))
            if not result[0]:
                # TRANSLATORS: Please leave %s as it is,
                # because it is needed by the program.
                # Thank you for contributing to this project.
...
''') % (self.configuration, result[1])
                ...
                exit(int(common.JOBSTATUS().ABORTED))
            #
            self.stderr.write(_(u'''Successfully generated a new cut list\n\n'''))
        #
        return
#
    def _set_sql_starttime(self,):
        ...
        if self.OWN_VERSION[1] >= 26:
            if str(type(self.recorded.starttime)).find("MythTV.utility.dt.datetime") == -1:
                # TRANSLATORS: Please leave %s as it is,
                # because it is needed by the program.
                # Thank you for contributing to this project.
                verbage = _(
...
Your installed version has been detected as "%s", aborting script.''') % self.configuration['mythtv_version']
                self.logger.critical(verbage)
                self.stderr.write(verbage + u'\n')
                #
                ## Remove this recording's files from the working directory
                ...
                exit(int(common.JOBSTATUS().ABORTED))
            #
            self.configuration['SQL_starttime'] = self.configuration['starttime'].astimezone(self.configuration['starttime'].UTCTZ()).strftime(common.DATETIME_SQL_FORMAT)
            self.configuration['SQL_progstart'] = self.configuration['progstart'].astimezone(self.configuration['progstart'].UTCTZ()).strftime(common.DATETIME_SQL_FORMAT)
        else:
            self.configuration['SQL_starttime'] = self.configuration['starttime']
            self.configuration['SQL_progstart'] = self.configuration['progstart']
        #
        return
#
    def _get_recorder_details(self, ):
        ...
        if not len(channel) > 0:
            self.logger.info(_(u'''There is no channel record for chanid "%d", skipping getting recorder details.''' % self.recorded.chanid))
            return
        #
        ...
        if not len(cardinfo) > 0:
            self.logger.info(_(u'''There is no cardinfo record for sourceid "%d", skipping getting recorder details.''' % channel[0][0]))
            return
        #
        ...
        if not len(capturecard) > 0:
            self.logger.info(_(u'''There is no capturecard record for cardid "%d", skipping getting recorder details.''' % cardinfo[0][0]))
            return
        #
...
#
    def _get_program_genre(self, ):
        ...
        if not len(program) > 0:
            self.logger.info(_(u'''There is no program guide for this recorded video, skipping getting the genre.'''))
            return u""
        #
        return program[0][0]
#
    def is_unique(self, ):
        ...
        '''
        try:
            list(self.MythVideo.searchVideos(custom=(("filename=%s", self.configuration['export_path_file'] + u'.mkv'),)))[0]
        except IndexError:
            return False
        #
        return True
#
    def add_to_mythvideo(self, ):
        ...
        if not self.configuration['v024'] and self.configuration['inetref']:
            artworkArray = list(self.mythdb.searchArtwork(inetref=self.configuration['inetref']))
            ...
            if len(artworkArray):
                ...
                for artwork in artworkArray:
                    ...
                    if not self.configuration['season_num'] == artwork.season:
                        continue
                    for key in keys:
                        if key == 'coverart':
...
#
    def copy(self):
        ...
        #
        verbage = (_(u'''Copying "%s"''') % self.configuration['mkv_file'] + _(u" to myth://Videos@%s/%s") % (self.vid.host, self.vid.filename))
        self.logger.info(verbage)
        self.stdout.write(verbage + u'\n\n')
        #
        ...
        #
        self.logger.info(_(u"Transfer Complete %d seconds elapsed") % int(time.time()-stime))
        #
        return
#
    def _get_tv_metadata(self, series=None, episode=None, inetref=False, season_num=None, episode_num=None):
        ...
        try:
            if inetref:
                try:
                    self.metadata = self.ttvdb.grabInetref(inetref, season=season_num, episode=episode_num)
                    ...
                    self.logger.info(_(u'''Found TV series "%s" Episode "%s".''') % (self.metadata['title'], self.metadata['subtitle']))
                except StopIteration:
                    pass
            #
            if not self.metadata and series and episode:
                try:
                    self.metadata = self.ttvdb.sortedSearch(series, episode)
                    self.logger.info(_(u'''Found TV series "%s" Episode "%s".''') % (self.metadata[0]['title'], self.metadata[0]['subtitle']))
                except IndexError:
                    self.logger.info(_(u'''Falling back to generic export.'''))
                    if not self.configuration['v024']:
                        self.metadata = self.recorded.exportMetadata()
                    return
                #
                if (len(self.metadata) > 1) & (self.metadata[0].levenshtein > 0):
                    self.logger.info(_(u'''Falling back to generic export.'''))
                    if not self.configuration['v024']:
                        self.metadata = self.recorded.exportMetadata()
                else:
            ...
            self.logger.info(_(u'''MythError, falling back to generic export. Error %s''') % errmsg)
            if not self.configuration['v024']:
                self.metadata = self.recorded.exportMetadata()
        #
        return
        ...
        try:
            if inetref:
                try:
                    ...
                    self.logger.info(_(u'''Found Movie "%s".''') % (self.metadata['title']))
                    # Default to the first studio and category
                    if self.metadata.studios:
                        self.studio = self.metadata.studios[0]
                    if self.metadata.categories:
            ...
            if not self.metadata and title:
                try:
                    ...
                    self.logger.info(_(u'''Found Movie "%s".''') % (self.metadata[0]['title']))
                except IndexError:
                    self.logger.info(_(u'''Falling back to generic export.'''))
                    if not self.configuration['v024']:
                        self.metadata = self.recorded.exportMetadata()
...
#
    def calc_dd_blocks(self, starttime):
        ...
        if self.configuration['fps']:
            self.configuration['frame'] = int(starttime * self.configuration['fps'])
            #
            frame_offset = [seek.offset for seek in self.recorded.seek
                    if seek.type == 9 and seek.mark <= self.configuration['frame']][-1]
            #
            if frame_offset == None:
                self.logger.info(_(u'''There was no offset found for frame %(frame)s, returning start block equal to zero.''') % self.configuration)
        else:
            self.logger.info(_(u'''There is not FPS value to calculate an offset, returning start block equal to zero.'''))
        #
        return frame_offset
#
    def get_all_recording_data(self,):
        ...
        for table in recorded_data.keys():
            ...
            for key_starttime in ['SQL_starttime', 'SQL_progstart']:
                #
                self.configuration['SQL_start'] = self.configuration[key_starttime]
                sql_cmd = common.SQL_GET_OR_INSERT['get_sql'] % self.configuration
                cursor.execute(sql_cmd)
                records = cursor.fetchall()
                if len(records) > 0:
                    break
            #
            verbage = _(u'''Read %d records from the %s table for chanid "%s" with starttime "%s".''') % \
                        (len(records), table, self.configuration['chanid'], self.configuration['SQL_starttime'])
            self.logger.info(verbage)
            self.stdout.write(verbage + u'\n')
            #
...
#
    def insert_all_recording_data(self, recorded_data):
        ...
        if self.configuration['base_name'].find('_LOSSLESS_BUG') != -1:
            verbage = _(u'''Adjust the DB records as this is only a sample video file.''')
            self.logger.info(verbage)
            self.stdout.write(verbage + u'\n')
            recorded_data['recorded'][0]['basename'] = self.configuration['base_name']
            self.configuration['mkv_file'] = self.configuration['base_name']
            recorded_data['recorded'][0]['filesize'] = os.path.getsize(self.configuration['base_name'])
            #
            new_markuptable = []
            for markup_record in recorded_data['recordedmarkup']:
                if markup_record['type'] == 32:
                    fps_str = str(markup_record['data'])
                    self.configuration['fps'] = float(fps_str[:2] + '.' + fps_str[2:])
                elif markup_record['type'] == 33:
                    # Duration in millseconds
                    markup_record['data'] = recorded_data['video_duration'] * 1000
            ...
            for seekrecord in recorded_data['recordedseek']:
                if seekrecord['type'] == 9:
                    if seekrecord['mark'] == 0 and seekrecord['offset'] == 0:
                    ...
                    if recorded_data['sample_starttime'] == 0:
                        if timestamp > recorded_data['video_duration']:
                            continue
                        new_seektable.append(seekrecord)
                    elif timestamp < recorded_data['sample_starttime'] or timestamp > recorded_data['video_duration']:
                        previous_offset = seekrecord['offset']
                    else:
                        seekrecord['offset'] = seekrecord['offset'] - previous_offset
                        new_seektable.append(seekrecord)
                else:
                    new_seektable.append(seekrecord)
        ...
        for table in common.SQL_GET_OR_INSERT['insert_sql'].keys():
            ...
            #
            del_data['SQL_start'] = recorded_data['recorded'][0]['starttime']
            if table in common.SQL_GET_OR_INSERT['progstart']:
                del_data['SQL_start'] = recorded_data['recorded'][0]['progstart']
...
Error: %s''') % (sql_cmd, errmsg)
        ...
        for table in common.SQL_GET_OR_INSERT['insert_sql'].keys():
            #
            verbage = _(u'''Inserting %d records in the %s table.''') % (len(recorded_data[table]), table)
            self.logger.info(verbage)
            self.stdout.write(verbage + u'\n')
            # Get the list of fields in this table record
            if 'All' in common.SQL_GET_OR_INSERT['insert_sql'][table]:
            ...
            for record in recorded_data[table]:
                sql_cmd = common.SQL_GET_OR_INSERT['insert_sql'][table][version][1] % record
                try:
                    cursor.execute(sql_cmd)
                except Exception as errmsg:
...
Error: "%s"''') % (sql_cmd, errmsg))
        ...
        cursor.close()
        #
        verbage = _(u'''All DB records inserted.''')
        self.logger.info(verbage)
        self.stdout.write(verbage + u'\n')
        #
        return recorded_data
#
    def add_video_to_storage_group(self, db_record, videofile):
        ...
        #
        verbage = _(u"Copying %s") % (videofile) + u" to myth://%s@%s/%s" % (db_record.storagegroup, db_record.hostname, db_record.basename)
        self.logger.info(verbage)
        self.stdout.write(verbage + u'\n\n')
...
#
    def match_and_remove(self,):
        ...
        #
        if match_found:
            program = self.mythbeconn.getRecording(chanid=self.recorded.chanid, starttime=self.recorded.starttime)
            if program == None:
                verbage = (_(u'''
There is no Program record for the recording:
Title: "%s" on channel %s with start time "%s"
This recording cannot be deleted, aborting script''') % (self.recorded.title, (u'%s' % self.recorded.chanid)[1:], self.recorded.starttime))
                self.logger.info(verbage)
                self.stdout.write(verbage + u'\n')
                #
...
Title: "%s" on channel %s with start time "%s"
''') % (self.recorded.title, (u'%s' % self.recorded.chanid)[1:], self.recorded.starttime))
                self.logger.info(verbage)
                self.stdout.write(verbage + u'\n')
            else:
                verbage = (_(u'''
        ...
        if self.configuration['remove_recorded']:
            for match in self.configuration['remove_recorded']:
                if match[0] == True and match[1] == True:
                    ...
                    match_found = True
                    break
                elif match[1] == False and (u'%s' % self.recorded.chanid)[1:] == match[0]:
                    match_found = True
                    break
                elif (u'%s' % self.recorded.chanid)[1:] == match[0] and filter(is_not_punct_char, self.recorded.title.strip().lower()) == match[1]:
                    match_found = True
                    break
        #
        return match_found
